...
uint8_t I2C_ReadByteTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr, uint16_t u16DataAddr)
{
    ...
    while(u8Xfering && (u8Err == 0))
    {
		...
		//printf("I2C_ReadByteTwoRegs*status= 0x%x,%x,%x,%x\n",I2C_GET_STATUS(i2c),u8Xfering,u8Err,(i2c)->DAT);
        switch(I2C_GET_STATUS(i2c))
        {
            case 0x08:
		//		printf("(u8SlaveAddr << 1 | 0x00)= 0x%x\n",(u8SlaveAddr << 1 | 0x00));
                I2C_SET_DATA(i2c, (u8SlaveAddr << 1 | 0x00));               /* Write SLA+W to Register I2CDAT */
                u8Ctrl = I2C_CTL_SI;                                      /* Clear SI */
                break;
            case 0x18:                                                      /* Slave Address ACK */
		//		printf("(u16DataAddr & 0xFF00) >> 8= 0x%x\n",(u16DataAddr & 0xFF00) >> 8);
                I2C_SET_DATA(i2c, (uint8_t)((u16DataAddr & 0xFF00) >> 8));    /* Write Hi byte address of register */
                break;
            case 0x20:                                                      /* Slave Address NACK */
            case 0x30:                                                      /* Master transmit data NACK */
		...
		//			printf("(u16DataAddr & 0xFF)= 0x%x\n",(u16DataAddr & 0xFF));
                ...
                break;
            case 0x10:
		//		printf("((u8SlaveAddr << 1) | 0x01)= 0x%x\n",((u8SlaveAddr << 1) | 0x01));
